...
const WhoAmI: React.FC = () => {
  ...
  // 1. Animación de la Marquesina (Se detiene en Modo Rendimiento)
  useGSAP(() => {
    if (animationsEnabled) {
      ...
      gsap.set(".marquee-content", { xPercent: 0 });
    }
  }, { scope: containerRef, dependencies: [animationsEnabled] });

  // 2. Animaciones de Revelado al scrollear (Blindadas para React 18)
  useGSAP(() => {
    // Usamos .from() que es más eficiente para animaciones de entrada 'revelado'
    const tl = gsap.timeline({
      scrollTrigger: { 
        trigger: containerRef.current, // FIX: Trigger estable
        start: "top 75%", 
        once: true 
      }
    });

    // A. La tarjeta entra desde la izquierda
    tl.from(".profile-column", { 
      opacity: 0, 
      x: -100, 
      duration: 1, 
      ease: "power3.out" 
    })
    // B. Textos entran desde la derecha en cascada
    .from(".about-text-element", { 
      opacity: 0, 
      x: 50, 
      duration: 0.8, 
      stagger: 0.15, 
      ease: "power3.out", 
      clearProps: "opacity,transform" // Limpiamos solo lo justo para React
    }, "-=0.7")
    // C. Insignias saltan desde abajo ("pop")
    .from(".about-badge-wrapper", { 
      opacity: 0, 
      y: 30, 
      scale: 0.8, 
      duration: 0.6, 
      ease: "back.out(1.5)", 
      stagger: 0.1, 
      clearProps: "all" // CRÍTICO: Limpiamos props para que el hover de Tailwind funcione
    }, "-=0.4");

  }, { scope: containerRef }); 

  return (
    <section ref={containerRef} id="about" className="about-section relative min-h-screen bg-[#02050D] text-white pt-32 pb-24 md:pt-40 md:pb-32 overflow-hidden flex items-center">
      ...
      <div className="container mx-auto px-6 relative z-20 w-full max-w-7xl mt-10 lg:mt-0">
        <div className="flex flex-col lg:flex-row items-center justify-center gap-12 lg:gap-20 w-full">
          ...
          {/* COLUMNA IZQUIERDA: Tarjeta de Perfil y Orbe */}
          {/* ELIMINADA la clase opacity-0 */}
          ...
          {/* COLUMNA DERECHA: Textos e Insignias */}
          <div className="w-full lg:w-1/2 flex flex-col items-center lg:items-start justify-center text-center lg:text-left max-w-[600px] mx-auto will-change-transform">
            ...
            {/* ELIMINADAS las clases opacity-0 de los siguientes elementos */}
